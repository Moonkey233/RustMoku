param(
    ...
    [string]$Dataset, [string]$OpeningDb, [string]$Device = 'cpu', [int]$BatchSize,
...
foreach ($pair in @(@('opening-db',$OpeningDb),@('dataset',$Dataset),@('resume',$Resume),@('teacher-model',$TeacherModel),@('teacher-profile',$TeacherProfile))) {
