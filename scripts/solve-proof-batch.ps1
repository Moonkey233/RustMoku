param(
    ...
    [long]$Nodes = 1000, [double]$Seconds = 60, [Alias('RamMiB')][int]$SqliteCacheMiB = 32, [long]$DiskMiB = 1024,
...
$arguments = @('-X','utf8','-m','offline.shards',$Command,'--engine',$Engine,'--manifest',$Manifest,
    ...
    '--nodes',"$Nodes",'--seconds',"$Seconds",'--sqlite-cache-mib',"$SqliteCacheMiB",'--disk-mib',"$DiskMiB")
